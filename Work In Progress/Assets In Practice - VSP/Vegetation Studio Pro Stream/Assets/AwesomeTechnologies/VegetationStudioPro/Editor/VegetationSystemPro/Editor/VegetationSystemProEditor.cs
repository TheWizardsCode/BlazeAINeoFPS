...
namespace AwesomeTechnologies.VegetationSystem
{
    ...
    public class VegetationSystemProEditor : VegetationStudioProBaseEditor
    {
        ...
        private static readonly string[] RenderingLayerMaskNames =
        {
        ...
        void ConfirmBillboardColorSpace()
        {
         ...
         void ConfirmBillboardVersionNumber()
        {
            bool needRegenerate = false;
            ...
            for (int j = 0; j <= _vegetationSystemPro.VegetationPackageProList.Count - 1; j++)
            {
                ...
                for (int i = 0; i <= vegetationPackagePro.VegetationInfoList.Count - 1; i++)
                {
                    ...
                    if (vegetationItemInfo.VegetationType != VegetationType.Tree) continue;
                    if (vegetationItemInfo.UseBillboards == false) continue;
                    if (vegetationItemInfo.BillboardVersion == BillboardAtlasRenderer.BillboardVersion) continue;
                    if (vegetationItemInfo.VegetationPrefab == null) continue;
            ...
            {
                if (EditorUtility.DisplayDialog("Vegetation Studio - Tree billboards",
                    "Billboards was generated with an earlier version of the atlas generator and have to be regenerated.", "Regenerate"))
                {
                    for (int j = 0; j <= _vegetationSystemPro.VegetationPackageProList.Count - 1; j++)
                    {
                        VegetationPackagePro vegetationPackagePro =
        ...
        void DrawAdvancedInspector()
        {
            ...
            GUILayout.BeginVertical("box");
            EditorGUILayout.LabelField("Cache management (experimental)", LabelStyle);
            
            _vegetationSystemPro.UseCacheCompacter = EditorGUILayout.Toggle("Compact cache",
                _vegetationSystemPro.UseCacheCompacter);
            
            EditorGUILayout.HelpBox("When enabled any vegetation cell out of range of any camera will clear its cache at the end of the frame.", MessageType.Info);
            GUILayout.EndVertical();
        }
...
void DrawEnvironmentInspector()
            ...
            int vegetationPackageIndex, int vegetationItemIndex)
        {
            ...
            if (_vegetationSystemPro.ShowLODMenu)
            {
                ...
                vegetationItemInfoPro.EnableCrossFade =
                    EditorGUILayout.Toggle("Enable LOD Cross fade",
                        vegetationItemInfoPro.EnableCrossFade);
                if (vegetationItemInfoPro.EnableCrossFade)
                {
                    EditorGUILayout.HelpBox("The shader has to support unitys crossfade system using LOD_FADE_CROSSFADE pragma for crossfade to work. Indirect shaders need custom code in the setup function. Instanced mode on HD SRP is not supported.",
                        MessageType.Info);
                }
            ...
            VegetationItemInfoPro vegetationItemInfoPro)
        {
            ...
            if (vegetationItemInfoPro.UseBillboards)
            {
                ...
                vegetationItemInfoPro.BillboardShadowOffset = EditorGUILayout.Slider("Shadow offset",
                    vegetationItemInfoPro.BillboardShadowOffset, 0f, 6f);
                EditorGUILayout.HelpBox("Adjusts shadow plane position for 3D billboards.",
                    MessageType.Info);
                ...
                vegetationItemInfoPro.BillboardWindSpeed = EditorGUILayout.Slider("Wind speed factor",
                    vegetationItemInfoPro.BillboardWindSpeed, 0, 5f);
                EditorGUILayout.HelpBox("Billboard wind can be enabled on custom shaders with a shadercontroler.Base wind speed is set on the environment tab. HDWind",
                    MessageType.Info);
                
                vegetationItemInfoPro.BillboardFadeDistance = EditorGUILayout.Slider("Far fade distance",
                    vegetationItemInfoPro.BillboardFadeDistance, 0f, 100f);
                vegetationItemInfoPro.UseBillboardFade = EditorGUILayout.Toggle("Enable far distance fade",
                    vegetationItemInfoPro.UseBillboardFade);
                EditorGUILayout.HelpBox("Enable to fade out the the billboard over " +vegetationItemInfoPro.BillboardFadeDistance.ToString("F0") +" meters from the max distance",
                    MessageType.Info);
                ...
                vegetationItemInfoPro.OverrideShaderController = EditorGUILayout.Toggle("Override shader controller",
                    vegetationItemInfoPro.OverrideShaderController);

                if (vegetationItemInfoPro.OverrideShaderController)
                {
                    vegetationItemInfoPro.UseBillboardSnow = EditorGUILayout.Toggle("Enable billboard snow",
                        vegetationItemInfoPro.UseBillboardSnow);      
                    vegetationItemInfoPro.UseBillboardWind = EditorGUILayout.Toggle("Enable billboard wind",
                        vegetationItemInfoPro.UseBillboardWind);      
                }
                EditorGUILayout.HelpBox("Enable to override shader controllers snow and wind setting", MessageType.Info);
        ...
        void DrawDebugInspector()
        {
            ...
            EditorGUI.BeginChangeCheck();
            _vegetationSystemPro.VegetationRenderSettings.ShowLODDebug =
                EditorGUILayout.Toggle("Show LODs.", _vegetationSystemPro.VegetationRenderSettings.ShowLODDebug);
            EditorGUILayout.HelpBox("Preview needs instanced indirect rendering and only works in playmode with compatible shaders.", MessageType.Info);
            if (EditorGUI.EndChangeCheck())
            {
                _vegetationSystemPro.RefreshMaterials();
                SetSceneDirty();
        ...
        void DrawRenderInspector()
        {
            ...
            EditorGUI.BeginChangeCheck(); 
            _vegetationSystemPro.VegetationRenderSettings.EnableSinglePassInstancedVR = EditorGUILayout.Toggle("Enable single pass instanced VR",
                _vegetationSystemPro.VegetationRenderSettings.EnableSinglePassInstancedVR);
            ...
            EditorGUILayout.HelpBox("Enable this if you are using single pass instanced in VR", MessageType.Info);
            GUILayout.EndVertical();
            
            
            GUILayout.BeginVertical("box");
            EditorGUILayout.LabelField("Light layers", LabelStyle);
           
            _vegetationSystemPro.VegetationRenderSettings.EnableInstancedRenderingLayers = EditorGUILayout.Toggle("Enable instanced light layers",
                _vegetationSystemPro.VegetationRenderSettings.EnableInstancedRenderingLayers);
            EditorGUI.BeginChangeCheck(); 
            _vegetationSystemPro.VegetationRenderSettings.RenderingLayerMask = EditorGUILayout.MaskField(_vegetationSystemPro.VegetationRenderSettings.RenderingLayerMask, RenderingLayerMaskNames);
           
            if (EditorGUI.EndChangeCheck())
            {
                EditorUtility.SetDirty(target);
                _vegetationSystemPro.SetupRenderingLayerData();
            }
            EditorGUILayout.HelpBox("Select the HD SRP light layers you want to use for the vegetation. For instanced indirect the shaders need an extra setup step in the setup function. ", MessageType.Info);
            GUILayout.EndVertical();
            
            GUILayout.BeginVertical("box");
            EditorGUILayout.LabelField("Crossfade", LabelStyle);
            
            EditorGUI.BeginChangeCheck(); 
            _vegetationSystemPro.VegetationRenderSettings.CrossFadeDistance = EditorGUILayout.Slider(
                "Cross fade distance", _vegetationSystemPro.VegetationRenderSettings.CrossFadeDistance, 0, 30);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            EditorGUILayout.HelpBox("Crossfade needs to be enabled on each item. Instanced indirect shaders needs custom code in the setup function to pass the Unity_LODfade data. ", MessageType.Info);
            GUILayout.EndVertical();
        }
